...
def Db():
    ...
    for j in range(18):
        ...
        for i in range(0+46*j,46+46*j):
            ...
            first_occurrence = ((max_pos_acc + net_pos_acc) > 0) & (first_day == -1)
        ...
        output_ds.to_netcdf(f'Dbedrock_{2003+j}_temp1.nc')
...
def cdo_add(filename1, filename2, filename3):
    subprocess.run(f"cdo add {filename1} {filename2} {filename3}", shell=True, check=True)
...
def Db_Dr():
    for j in range(18):
        print(j+2003)
        subprocess.run(f"cdo -b F32 -P 12 --no_remap_weights remapbil,mask1.nc Dbedrock_{2003+j}_temp1.nc Dbedrock_{2003+j}_temp2.nc", shell=True, check=True)
    
    Parallel(n_jobs=5)(delayed(cdo_mul)(f"Dbedrock_{2003+j}_temp2.nc", "mask123.nc", f"Dbedrock_{2003+j}.nc") for j in tqdm(range(18)))

    Parallel(n_jobs=5)(delayed(cdo_add)(f"Dbedrock_{2003+j}.nc", "Ssoil.nc", f"Dr_{2003+j}_temp1.nc") for j in tqdm(range(18)))
    
    Parallel(n_jobs=5)(delayed(cdo_mul)(f"Dr_{2003+j}_temp1.nc", "mask123.nc", f"Dr_{2003+j}.nc") for j in tqdm(range(18)))

# Delete the intermediate data to save memory
@timer
def delete():
    os.system('rm -rf Dbedrock_*_temp1.nc')
    os.system('rm -rf Dbedrock_*_temp2.nc')

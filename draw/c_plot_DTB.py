...
shp_path   =  '/home/xuxh22/stu01/Bedrock/data/US/States/'
shp = gpd.read_file(shp_path+'Iowa.shp')
...
fig = plt.figure(figsize=(18, 4), dpi=2000)
fig.subplots_adjust(left=0.05, right=0.98, 
                bottom=0.14, top=0.95, hspace=0, wspace=0.05) 
gs = GridSpec(1, 4)

test = xr.open_dataset(f'{path}Iowa/Iowa.nc')
lat = test['lat']
lon = test['lon']
...
def draw1():
    image = xr.open_dataset(f'{path}gNATSGO/Bedrock_US_gNATSGO_90m-2.nc').sel(lon=slice(xmin,xmax),lat=slice(ymin,ymax))
    s = image['Band1']
...
def draw2():
    image = xr.open_dataset(f'{path}Iowa/Iowa.nc').sel(lon=slice(xmin,xmax),lat=slice(ymin,ymax))
    s = image['Band1']
    s = s.salem.roi(shape=shp)
    name = 'Iowa'
...
def draw3():
    image = xr.open_dataset(f'{path}Soilgrids/DTB_temp2.nc').sel(lon=slice(xmin,xmax),lat=slice(ymin,ymax))
    s = image['Band1']
    s = s.salem.roi(shape=shp)
    name = 'SoilGrids250m'
...
def draw4():
    image = xr.open_dataset(f'{path}Soilgrids_Iowa/DTB_temp3.nc').sel(lon=slice(xmin,xmax),lat=slice(ymin,ymax))
    s = image['Band1']
    s = s.salem.roi(shape=shp)
    name = 'SoilGrids250m_rev'

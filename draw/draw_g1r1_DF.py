...
def data_process(region):
    image = xr.open_dataset(f'{data_path}/Dbedrock_Frequency.nc').sel(lon=slice(region[0],region[1]),lat=slice(region[2],region[3]))
    s = image['Dr'][::-1,:]
    return s
...
def draw(region,name,level,cmap):
    ...
    if region[0] == -180:
        set_other(ax)
        plt.savefig(f"{fig_path}/{name[1]}_{name[2]}.png")
    else:
        plt.savefig(f"{fig_path}/{name[1]}_{name[2]}_{name[3]}.png")

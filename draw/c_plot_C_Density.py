...
def plot(ax, xmin, xmax, ymin, ymax, name, level, cmap):
    image = xr.open_dataset(f'{path}/C_Density/{name[0]}.nc').sel(lon=slice(xmin,xmax),lat=slice(ymin,ymax))
    s = image[f'{name[1]}']
...
level1 = np.arange(0,1500,150)
level2 = np.arange(0,1000,100)
...
name = ['aboveground_biomass_carbon_remap', 'Band1', 'C_a', 'C_a']
level = level1
cmap = cmap1
draw(name,level,cmap)

name = ['belowground_biomass_carbon_remap', 'Band1', 'C_b', 'C_b']
level = level2
cmap = cmap1
draw(name,level,cmap)

...
shp = gpd.GeoDataFrame.from_file(shp_path+'/US_conus/conus_ard_grid.shp')

df = pd.read_csv(f'{data_path}site.csv', encoding='latin-1')
df1 = df.copy()
...
def map():
    ...
    df2['Measure'] = df2['Measure'].astype(str)
    df2['mask'] = df2['mask'].astype(str)
    ...
    plot_base.save(f'{fig_path}s5_test.png')
...
def DTB():
    ...
    for i in range(7):
        ...
        base_plot.save(f'{fig_path}site{i}.png')
...
def Sbedrock():
    ...
    base_plot.save(f'{fig_path}site_Sbedrock.png')
    return df1_rev  
...
def point_map(df_grid):
    ...
    plot_base.save(f'{fig_path}test.png', dpi=300) 

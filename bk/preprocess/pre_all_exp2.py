...
def mask1():
    ...
    os.system(f'cdo -setrtoc2,0,200,1,nan DTB_temp1.nc mask1.nc')
    print("mask1_v4已完成")
# -------------------------------- mask1 ------------------------------------------------

# -------------------------------- mask all ------------------------------------------------
def mask():
    dir_path = path1+'mask/mask_v4/'
    mask1_path = path1+'mask1/mask1_v4/'
...
def Ssoil():
    dir_path2 = DTB_layer()
    # dir_path2 = path1+' mask1/mask1_v4/'
    dir_path = path1+'Ssoil/Ssoil_v4/'
    os.makedirs(dir_path, exist_ok=True)
    os.chdir(dir_path)

    layer = [0, 5, 15, 30, 60, 100, 200, float('inf')]
    layer_depth_mm = [50, 100, 150, 300, 400, 1000]

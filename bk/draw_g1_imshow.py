...
os.makedirs(f'{fig_path}/global_map_1', exist_ok=True)
...
def data_process(name):
    image = xr.open_dataset(f'{data_path}/{name[0]}.nc4').sel(lon=slice(region[0],region[1]),lat=slice(region[2],region[3]))
    s = image[f'{name[1]}']
    ...
    if (name[1]=='Sbedrock') or (name[1]=='FD') or (name[1]=='Dr'):
...
@timer
def set_ax(name,ax,s,df2,cmap,level):
    if name[1] == 'Sbedrock':
...
def draw(name,level,cmap):
    ...
    fig,ax = set_fig()
    img = set_ax(name,ax,df1,df2,cmap,level)
    ...
    plt.savefig(f"{fig_path}//global_map_1/g1_{name[2]}.png")
    plt.close(fig)
...
def Sb():
    ...
    name = ['Sbedrock', 'Sbedrock', 'Sb_n2p', '$S_{{bedrock}}$ (mm)']
    level = level2
    cmap = cmap2
    draw(name,level,cmap)
    name = ['Sbedrock_tmp1', 'Sbedrock', 'Sb_n2p_nm', '$S_{{bedrock}}$ (mm)']
    level = level2
    cmap = cmap2
    draw(name,level,cmap)
...
def PR_ET_Q():
    name = ['PR_mean', 'tp', 'PR', 'Precipitation (mm)']
    level = np.arange(0,1600,400)
    cmap = cmaps.WhiteBlue
    draw(name,level,cmap)
    name = ['ET_mean', 'et', 'ET', 'Evapotranspiration (mm)']
    level = np.arange(0,1600,400)
    cmap = cmaps.WhiteBlue
    draw(name,level,cmap)
    name = ['Q_mean', 'tp', 'Q', 'Streamflow (mm)']
    level = np.arange(0,1600,400)
    cmap = cmaps.WhiteBlue
    draw(name,level,cmap)
...
def FD():
    name = ['S_FD', 'FD', 'FD', 'First Day']
    level = np.arange(0,900,150)
    cmap = cmaps.StepSeq25_r
    draw(name,level,cmap)
    # name = ['S_FD_mean', 'FD', 'FD_mean', 'First Day']
    # level = level5
    # cmap = cmaps.StepSeq25_r
    # draw(name,level,cmap)
...
def LH():
    for year in range(2003,2021):
        for mon in range(1,13):
            name = [f'LH_{year}_{mon}_tmp1', 'Ee', f'LH_{year}_{mon}', 'Latent Heat (W/$m^{{2}}$)']
